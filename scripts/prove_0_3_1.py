#!/usr/bin/env python3
"""Prove 0.3.1 on Mac/Windows: unit tests + CLI runner smoke + job store + MCP/cursor-sdk import."""
...
UV_MCP_WITH = ["--with", "mcp>=1.9,<2", "--with", "cursor-sdk"]
...
def prove_mcp_import() -> None:
    section("MCP import + tool registry + cursor-sdk")
    uv = shutil.which("uv")
    if not uv:
        try:
            ...
            raise SystemExit("mcp/cursor-sdk not installed and uv not on PATH") from exc
        ...
        print("cursor_sdk:", cursor_sdk.__name__)
        return
